...
void M_Dio_void_setPinValue(Dio_ChannelType ChannelId,Dio_LevelType Level)
{
	...
	else if (Level == DIO_LOW)
	{
		switch(port)
		{
		case DIO_PORTA : CLEAR_BIT( PORTA_REG,pin);break;
		case DIO_PORTB : CLEAR_BIT( PORTB_REG,pin);break;
		case DIO_PORTC : CLEAR_BIT( PORTC_REG,pin);break;
		case DIO_PORTD : CLEAR_BIT( PORTD_REG,pin);break;
		}
	}
